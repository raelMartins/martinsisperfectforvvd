...
export default function ChatFooter() {
  ...
  return (
    <footer
      className={`theme-fade relative z-20 w-full shrink-0 border-t backdrop-blur-md ${LAYOUT.footerHeightClass}`}
      style={{
    ...
    >
      <div className="flex h-full items-center gap-2.5 px-3 py-1.5 sm:gap-4 sm:px-5 md:gap-5 md:px-8 md:py-2">
        ...
        <div
          className="theme-fade flex h-9 min-w-0 flex-1 items-center rounded-full px-3 sm:h-11 sm:px-5 md:h-12 md:px-6 lg:h-[64px] lg:px-7"
          style={{
            backgroundColor: colors.composerBg,
            // Inset shadow rather than a border so the pill's metrics are
            // identical in both themes.
            boxShadow: `inset 0 0 0 1px ${colors.composerBorder}`,
          }}
        >
          <div
            ref={textColorRef}
